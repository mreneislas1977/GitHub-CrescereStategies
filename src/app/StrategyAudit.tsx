...
import { useModal } from './ModalContext'; // <--- Import the hook

const StrategyAudit = () => {
  const { openModal } = useModal(); // <--- Get the "switch"
    ...
    <section className="bg-[#014421] text-[#fdfbf5] py-20 px-6 border-t border-[#fdfbf5]/10">
      <div className="container mx-auto max-w-5xl flex flex-wrap items-center gap-12">
        ...
        <div className="flex-shrink-0 w-full md:w-[350px] bg-[#fdfbf5] text-[#014421] p-10 rounded-sm text-center shadow-2xl relative">
          ...
          <button 
            onClick={openModal} // <--- Calls the global popup
            className="w-full block bg-[#014421] hover:bg-[#022c16] text-[#fdfbf5] py-4 px-6 font-bold text-sm uppercase tracking-widest transition-colors relative z-10"
          >
            Schedule the Audit
          </button>
        </div>
